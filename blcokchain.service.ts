...
import { MORALIS } from 'src/common/constants';
...
export class BlockchainService {
    ...
    try {
      ...
      if (
        ...
        throw new BadRequestException(
          `Insufficient balance in wallet ${walletAddress}. Current balance: ${balanceEth} POL. Please add funds to complete the transaction.`,
        );
      }
  ...
  /**
   * Common helper used by controllers/jobs to assert sufficient wallet balance
   * for registering a CID. Throws BadRequestException if insufficient.
   */
  ...
  /**
   * Estimate total transaction cost for updateProperty(tokenId, cid)
   * Returns gas, gasPrice and total cost in wei (as bigint) and POL string
   */
  async estimateUpdatePropertyCost(
    tokenId: number,
    cid: string,
  ): Promise<{
    gasLimit: bigint;
    ...
    try {
      ...
      this.logger.error('Failed to estimate updateProperty gas:', error);
  ...
  async getPropertyOwnershipHistory(propertyId: string): Promise<{
    ...
    try {
      ...
      const moralisUrl = `https://deep-index.moralis.io/api/v2.2/nft/${contractAddress}/${tokenId}/transfers?chain=${MORALIS.CHAIN}&format=decimal`;
